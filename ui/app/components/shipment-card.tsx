...
import { type ShipmentEvent, type ShipmentRead } from "~/lib/client";
    ...
    return (
        <Card className="shadow-none" >
            ...
            <CardContent className="grid gap-4">
                <div className="flex items-center space-x-4 rounded-xl p-3.75 bg-gray-100 relative">
                    <div className="absolute left-20 top-0 bottom-0 w-0.5 bg-gray-300" />
                    <div data-line className={`absolute left-20 top-6 bottom-0 w-0.5 ${statusColor.bgColor}`} />
...
function TimelineEvent({ event, bgColor, outlineColor, hasOutline = false }: { event: ShipmentEvent, bgColor: string, outlineColor: string, hasOutline?: boolean }) {
    return (
        <div className="flex items-center gap-x-3.75">
            <p className="text-xs text-muted-foreground w-7.5">
                {event.created_at.split("T")[1].slice(0, 5)}
            </p>
            <div className={`w-10 h-10 ${bgColor} text-foreground flex items-center justify-center rounded-full ${hasOutline ? `outline-2 ${outlineColor} outline-offset-2` : ''}`}>
